...
class LULCCensus(db.Model):
    ...
    @classmethod
    def get_lulc_runoff(cls, block_id, district_id):
        query = db.session.query(
            func.sum(LULCCensus.lulc_area).label("catchment_area"),
            LULC.catchment.label("catchment")
        ...
        ).filter(Block.id == block_id, District.id == district_id, LULC.id < 15
        ).group_by(LULC.id)

        results = query.all()

        if results:
            json_data = [{'catchment':row.catchment, 'catchment_area':row.catchment_area} for row in results]
            return json_data
        return None
    ...
    @classmethod 
    def get_lulc_data(cls,block_id,district_id):
        query = db.session.query(
            func.round(func.sum(LULCCensus.lulc_area).cast(Numeric), 2).label('lulc_area'),
            LULC.id,
            LULC.display_name
