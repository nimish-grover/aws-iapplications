...
class BlockOrCensus:
    ...
    def get_human_data(cls, block_id, district_id, state_id):
        #return block data
        # HUMAN DATA WILL BE SOURCED FROM CENSUS 2011 AS PER LATEST CONCEPT DOCUMENT
    ...
    @classmethod
    def get_irrigation_data(cls, block_id, district_id, state_id):
        irrigation_water_requirement = BudgetData.get_irrigation_water_requirement(block_id, district_id)
        return irrigation_water_requirement, False
    ...
    def get_water_transfer(cls, block_id, district_id, state_id):
        is_approved = False
        water_transfer = BudgetData.get_other_water(block_id, district_id)
        return water_transfer, is_approved
    ...
    @classmethod
    def get_harvested_data(cls, block_id, district_id, state_id):
        is_approved = False
        surface_water_supply = BudgetData.get_harvested_runoff(block_id, district_id, state_id)
        return surface_water_supply, is_approved
    
    @classmethod
    def get_potential_runoff(cls, block_id, district_id, state_id):
        is_approved = False
        harvested_runoff = BudgetData.get_harvested_runoff(block_id, district_id, state_id)
        available_runoff = BudgetData.get_runoff(block_id, district_id)
        harvested = sum(item['value'] for item in harvested_runoff)
        available = sum(item['supply'] for item in available_runoff)
        potential_runoff = [
            {'name':'available', 'value': available},
            {'name':'harvested', 'value': harvested},
            {'name':'potential', 'value': available - harvested}
        ]
        return potential_runoff, is_approved
    @classmethod
    def get_rainfall_data(cls, block_id, district_id, state_id):
        ...
        return rainfall_data, is_approved
    ...
    def get_demand_side_data(cls, block_id, district_id, state_id):
        ...
        crops,is_approved = cls.get_irrigation_data(block_id, district_id,state_id)
